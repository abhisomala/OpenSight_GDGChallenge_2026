...
**Versus agentic browsers and assistants (e.g. ChatGPT Atlas):** these can perform multi-step browser actions and carry context across tasks, so "agentic" alone is not the distinction. The difference is who they are built for. Agent mode in tools like Atlas is designed to be supervised visually: it narrates what it is doing, pauses for the user to watch on sensitive steps, and assumes the user can see the screen and take over. That interaction model presumes sight. ChatGPT Atlas was rated **1 out of 10** for screen reader accessibility by Taylor Arndt, a blind accessibility expert and Chief Accessibility Officer at Techopolis *(Double Tap, October 2025)*. OpenSight is built to be operated entirely without sight: voice in, a spoken answer out, a wake word instead of a cursor, and no visual surface to monitor.
...
```mermaid
flowchart TD
    A([User speaks]):::io --> B[Deepgram Nova-2 STT · desktop]
    B --> C[Firebase Authentication · when enabled · user token, never a model key]
    C --> D[OpenSight backend · Cloud Run · FastAPI WebSocket · SessionMemory · router]
    D --> E[Gemini 2.5 Flash · Vertex AI · intent classification · preference extraction]

    E --> F[Shopping agent · Playwright → Amazon]:::agent
    E --> G[Research agent · SerpAPI → Google Scholar]:::agent
    E --> H[Calendar agent · Google Calendar API]:::agent
    E --> I[General agent · Google Custom Search + Gemini]:::agent
...
| Daily use intent | **80%** said they would use a voice-first interface daily or for specific tasks |
...
| **Vertex AI** | Production serving for Gemini 2.5 Flash, called server-side via Application Default Credentials: production-grade quota, autoscaling, and control |
...
Built for the **Google Developer Groups on Campus Solution Challenge 2026** · *Virginia Tech*