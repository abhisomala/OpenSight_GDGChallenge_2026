...
- `research.py` — `_is_followup` uses keyword matching instead of word count. "Open it" dead-end fixed. Mutable default arg fixed. Playwright removed — execution moved to `desktop_browser.py`. `search_scholar()` / `synthesize_research_response()` split for server+client protocol. `get_open_intent()` added for server.py. Paper deduplication by title added. `_extract_product_hint` rewritten as `async def` — LLM primary path (passes topic + top-3 titles, asks for 2–4 word Amazon phrase), stopword-strip fallback retained; `synthesize_research_response` made `async` and awaited in server.py.
...
- `desktop_browser.py`, `agents/shopping.py` — `_clean_query` now strips budget phrases (`under $N`, `less than $N`, `below $N`, `for under $N`, `cheaper than $N`, `no more than $N`, `at most $N`, `up to $N`, `$N or less`, `$N or under`) before building the Amazon search URL, preventing price literals from appearing in `amazon.com/s?k=` and causing junk results or 503 errors. `_extract_budget` is unaffected — it always receives the raw query.
- `server.py`, `requirements.txt`, `.env.example` — Firebase Anonymous Auth gate added behind `REQUIRE_AUTH` env var (default off). When unset, the endpoint is byte-for-byte unchanged; no firebase import runs. When `REQUIRE_AUTH=true`, the first message on each connection must be `{"type":"auth","token":"<firebase_id_token>"}` — verified via `firebase_admin.auth.verify_id_token()` (lazy init, ADC + `GOOGLE_CLOUD_PROJECT`); failures close with code 4001. Added `firebase-admin>=6.0.0` to requirements.txt. Client code (agent.py) is untouched.
