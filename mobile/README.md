...
- A reachable OpenSight backend. By default the app uses the deployed Cloud Run
  backend, so nothing needs to run locally; only local development against a
  laptop-hosted backend needs port `8080` (see "Start the backend").

---

## 1. Point the client at the backend (`engineUrl`)

`engineUrl` in `lib/config.dart` selects the backend:

- **Deployed Cloud Run backend (default):**
  `wss://opensight-backend-348346331222.us-east1.run.app/ws`. Works from both
  emulators and physical devices with no local server; the scheme is `wss://`
  (TLS).

For local development against a backend running on your laptop, switch
`engineUrl` to the commented local line in `lib/config.dart`:

- **Android emulator:** `ws://10.0.2.2:8080/ws`. `10.0.2.2` is the
...
The local URLs use `ws://` (cleartext); the manifest already allows cleartext
traffic for this dev setup (not needed for the default `wss://` URL, but
harmless).
...
## 2b. Optional: Firebase Anonymous Auth (`requireAuth`)

`requireAuth` in `lib/config.dart` mirrors the backend's `REQUIRE_AUTH` flag and
is **off by default**. Keep the two in sync:

- **`requireAuth = false` (default):** the app touches **zero** Firebase at
  runtime — no `Firebase.initializeApp`, no sign-in, no auth frame. Behaves
  exactly as before.
- **`requireAuth = true`:** at startup `main()` calls
  `Firebase.initializeApp(...)` and `FirebaseAuth.instance.signInAnonymously()`
  (failures are logged and the app continues — it won't crash offline). Then, on
  **each** `/ws` connection, `EngineClient` sends
  `{"type":"auth","token":"<idToken>"}` as the **first** frame, before the
  `{"text": ...}` query. It does not wait for an ack: the server stays silent on
  success and only closes with code `4001` on failure.

This relies on `lib/firebase_options.dart` (generated by `flutterfire configure`)
and the `firebase_core` / `firebase_auth` packages in `pubspec.yaml`. Set the
backend's `REQUIRE_AUTH=true` for the server to verify the token.

## 3. Start the backend on `0.0.0.0:8080` (local development only)

Skip this step when using the default Cloud Run `engineUrl` — the deployed
backend is already running. For local development, in the OpenSight engine repo
(`GDG2`), serve the FastAPI app bound to all interfaces so the emulator/device
can reach it:
